...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "کاربری با چنین یوزرنیم یافت نشد!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "شما نمیتوانید مدیران/صاحب گروه/ادمین ها را اخراج کنید!")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "شمل نمیتوانید مدیران/صاحب گروه/ادمین ها را بن کنید!")
			return
        end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] به صورت جهانی بن شد!')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] به صورت جهانی آنبن شد!')
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید مدیران/صاحب گروه/ادمین ها را بن کنید!"
        end
...
if matches[1]:lower() == 'kick' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید مدیران/صاحب گروه/ادمین ها را اخراج کنید!"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] به صورت جهانی آنبن شد!'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
